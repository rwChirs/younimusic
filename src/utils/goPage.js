...
import { logClick, commonLogClick } from '../utils/common/logReport';
...
export default function toUrlByType({
  ...
  // 鲜橙新埋点
  if (buriedPointVo && buriedPointVo.pageId) {
    commonLogClick({ action, buriedPointVo });
  }

  //埋点
  if (action.clsTag) logClick({ eid: action.clsTag });
  ...
  switch (action.urlType) {
    ...
    case '210':
      ...
      break;
    case '220':
      (toUrl = `/pages/bill/bill-detail/index?${
        action.toUrl
  ...
  if (goH5) {
    ...
    if (toUrl.indexOf('m.jd.com/immersive/inviteHasGifts') > -1) {
      Taro.navigateTo({
        url: `/pages-a/invitation/index?${action.toUrl}&storeId=${storeId ||
          ''}`,
      });
      return;
    }
    if (toUrl.indexOf('m.jd.com/new-customer') > -1) {
      Taro.navigateTo({
        url: `/pages/secondaryActivity/newcustomer/index?${toUrl.slice(
          ...
          ''}&storeId=${storeId || ''}&fromPage=${action && action.fromPage}`,
      });
      return;
    }
