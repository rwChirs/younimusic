...
export default class ProductItem extends Component {
  ...
  addCart = (ev) => {
    ...
    onAddCart(data);
    if (!data.isPop && false) {
      const query = this.$scope.createSelectorQuery();
      query.select(`#product-image-${data.skuId}`).boundingClientRect();
      query.exec((imgInfo) => {
        if (imgInfo[0]) {
          this.setState({
            showFlyToCart: true,
            ...
            startPos: {
              ...
              y: Number.parseInt(imgInfo[0].top),
            },
          });
        }
      });
  ...
  render() {
    ...
    return (
      <View
        className={`product-item1 product-item-${handleType(type)}`}
        style={itemStyle}
        onClick={this.goDetail}
        id={`goods-item-x-${data.skuId}`}
      >
