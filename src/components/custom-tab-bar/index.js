...
export default class CustomTabBar extends Component {
  ...
  onSwitchTab(index) {
    ...
    if (index === 0) {
      ...
      if (index === 1) {
        // 分类
        eventId = 'MiniAPP_BottomNavigation_Classification';
        eventName = '小程序首页底部导航分类按钮';
      } else if (index === 2) {
        // 7Club
        eventId = 'MiniAPP_BottomNavigation_7CLUB';
        eventName = '小程序首页底部导航7CLUB按钮';
      } else if (index === 4) {
        // 个人中心
        eventId = 'MiniAPP_BottomNavigation_PersonalCenter';
        eventName = '小程序首页底部导航个人中心按钮';
      }
      /*
       * https://cf.jd.com/pages/viewpage.action?pageId=379942859
       */
    ...
    utils.navigateToH5({
      page:
        ...
        `/cart.html?from=miniapp&source=index&storeId=${
          this.state.storeId
        }&uuid=${uuid}&lat=${lbsData.lat}&lng=${
          lbsData.lon
        }&tenantId=${lbsData && lbsData.tenantId}`,
    });
